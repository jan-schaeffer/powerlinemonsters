...
foreach treatment of local treatments {
	foreach party of local parties{
		eststo `treatment'_`party': qui reg `party' i.`treatment'##i.post_2013 i.state_id i.wahlkreis i.year, cluster(ags)
	}
	*testparm i.year 
	*testparm i.state_id 
	*testparm i.wahlkreis
	...
	drop(0.treated_* 0.post_* 1.treated_*#0.post_*)
	coeflabels(1.`treatment' "`treatment'" 1.post_2013 "post_2013" 1.`treatment'#1.post_2013 "`treatment'*post_2013" _cons "Constant")
	indicate("Year FE = *year" "State FE = *state_id" "Constituency FE = *wahlkreis")
...
foreach treatment of local treatments {
	foreach party of local parties{
		eststo `treatment'_`party': qui reg `party' i.`treatment'##i.post_2013 pop_density female foreign unemployed lavg_income avg_age catholic, cluster(ags)
	}	
	#delimit;
	...
	addnote("Note: These are linear models. Standard errors in parentheses are clustered at the municipality level.")
	stats(N_clust N, fmt(0 0) labels("Municipalities" "Observations")) replace;
...
foreach treatment of local treatments {
	foreach party of local parties{
		eststo `treatment'_`party': qui reg `party' i.`treatment'##i.post_2009 pop_density female foreign unemployed lavg_income avg_age catholic, cluster(ags)
	}	
	#delimit;
